...
classes = [x.rstrip() for x in open(
    os.path.join(BASE_DIR, 'meta_data/class_names.txt'))]
...
class2color = {'ceiling':	[0, 255, 0],
                 ...
                 'board':       [200, 200, 200],
                 'clutter':     [50, 50, 50]}
easy_view_labels = [7, 8, 9, 10, 11, 1]
...
def point_label_to_obj(input_filename, out_filename, label_color=True, easy_view=False, no_wall=False):
    ...
    for i in range(data.shape[0]):
        ...
        if easy_view and (label[i] not in easy_view_labels):
            continue
        if no_wall and ((label[i] == 2) or (label[i] == 0)):
            continue
        if label_color:
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    if not random_sample:
        ...
        num_block_y = int(np.ceil(collect_point_label(
            limit[1] - block_size) / stride)) + 1
...
def bbox_label_to_obj(input_filename, out_filename_prefix, easy_view=False):
    ...
    Args:
        ...
        out_filename_prefix: OBJ filename prefix,
            visualize object by label2color
    ...
    for i in range(bbox.shape[0]):
        ...
        obj_filename = out_filename_prefix+'_' + \
            classes[label[i]]+'_'+str(ins_cnt)+'.obj'
        mtl_filename = out_filename_prefix+'_' + \
            classes[label[i]]+'_'+str(ins_cnt)+'.mtl'
        ...
        y = bbox[i, 1]
        z = bbox[i, 2]
        color = np.array(label2color[label[i]], dtype=float) / 255.0
...
def bbox_label_to_obj_room(input_filename, out_filename_prefix, easy_view=False, permute=None, center=False, exclude_table=False):
    ...
    for i in range(bbox.shape[0]):
        if easy_view and (label[i] not in easy_view_labels):
            continue
        if exclude_table and label[i] == classes.index('table'):
            continue
