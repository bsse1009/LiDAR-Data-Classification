...
all_file_prefixes = train_file_prefixes + \
    validation_file_prefixes + test_file_prefixes
train_full_file_prefixes = train_file_prefixes + validation_file_prefixes

map_name_to_file_prefixes = {
    "train": train_file_prefixes,
    "train_full": train_full_file_prefixes,
...
class SemanticFileData:
    def __init__(
        ...
        if has_label:
            self.labels = load_labels(file_path_without_ext + "_labels.npz")
            self.labels = self.labels[self.labels.files[0]]
        else:
    ...
    def _get_fix_sized_sample_mask(self, points, num_points_per_sample):
        ...
        if len(points) - num_points_per_sample > 0:
            ...
            false_array = np.zeros(
                len(points) - num_points_per_sample, dtype=bool)
            ...
            while len(sample_mask) < num_points_per_sample:
                sample_mask = np.concatenate(
                    (sample_mask, sample_mask), axis=0)
    ...
    def _extract_z_box(self, center_point):
        ...
        scene_z_size = np.max(self.points, axis=0)[
            2] - np.min(self.points, axis=0)[2]
...
class SemanticDataset(Dataset):
    def __init__(
        self, num_points_per_sample, split, use_color, box_size_x, box_size_y, path, is_training=True
    ):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_color:
            data = np.hstack((points_centered, colors))
        else:
    ...
    def __len__(self):
        ...
        return int(
            np.sum(list_num_points)/self.num_points_per_sample)
...
if __name__ == '__main__':
    ###########################Checking if Dataset serve our goals ##############################
    parser = argparse.ArgumentParser()
    parser.add_argument("--train_set", default="train",
                        help="train, train_full")
    parser.add_argument(
        "--config_file", default="semantic.json", help="config file path")
    ...
    print('point number batch:', TRAIN_DATASET.get_num_batches(PARAMS["batch_size"]))
    print('point data size:', TRAIN_DATASET.__len__())
    print('point data 0 shape:', TRAIN_DATASET.__getitem__(0)[0].shape)
    print('point label 0 shape:', TRAIN_DATASET.__getitem__(0)[1].shape)
    # print('point color 0 shape:', TRAIN_DATASET.__getitem__(0)[2].shape)
    # print('point weight 0 shape:', TRAIN_DATASET.__getitem__(0)[3].shape)

    dataloader = DataLoader(TRAIN_DATASET, batch_size=4, shuffle=True, num_workers=0)
    end = time.time()
    for i, (input_, target, _) in enumerate(dataloader):
        print('time: {}/{}--{}'.format(i+1, len(dataloader), time.time() - end))
        end = time.time()
        print(input_.shape)
        print(input_[0])
        if(i==3):
